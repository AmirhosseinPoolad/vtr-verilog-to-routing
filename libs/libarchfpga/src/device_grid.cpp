...
DeviceGrid::DeviceGrid(std::string_view grid_name,
    ...
    int interposer_region_counter = 0;
    for (size_t layer = 0; layer < num_layers; layer++) {
        ...
        vtr::NdMatrix<int, 2> layer_reduced_interposer_id_matrix({vertical_interposers.size() + 1, horizontal_interposers.size() + 1});

        for (size_t i = 0; i < vertical_interposers.size() + 1; i++) {
            for (size_t j = 0; j < horizontal_interposers.size() + 1; j++) {
                layer_reduced_interposer_id_matrix[i][j] = interposer_region_counter;
                interposer_region_counter++;
            }
        }

        // for (int j = (int)layer_reduced_interposer_id_matrix.dim_size(1) - 1; j >= 0; --j) {
        //     // Loop X normally (left to right)
        //     for (size_t i = 0; i < layer_reduced_interposer_id_matrix.dim_size(0); ++i) {
        //         VTR_LOG("%d ", layer_reduced_interposer_id_matrix[i][j]);
        //     }
        //     VTR_LOG("\n");
        // }

        vtr::NdMatrix<int, 2> layer_interposer_id_matrix = get_device_sized_matrix_from_reduced(x_size, y_size, horizontal_interposers, vertical_interposers, layer_reduced_interposer_id_matrix);
        interposer_id_matrix_.push_back(std::move(layer_interposer_id_matrix));
    }

    // for (const auto& layer_mat : interposer_id_matrix_) {
    //     for (int j = (int)layer_mat.dim_size(1) - 1; j >= 0; --j) {
    //         // Loop X normally (left to right)
    //         for (size_t i = 0; i < layer_mat.dim_size(0); ++i) {
    //             VTR_LOG("%d ", layer_mat[i][j]);
    //         }
    //         VTR_LOG("\n");
    //     }
    // }
...
bool DeviceGrid::are_locs_on_same_die(t_physical_tile_loc loc_a, t_physical_tile_loc loc_b) const {
    const int first_id = interposer_id_matrix_[loc_a.layer_num][loc_a.x - 1][loc_a.y - 1];
    const int second_id = interposer_id_matrix_[loc_b.layer_num][loc_b.x - 1][loc_b.y - 1];

    if (loc_a == t_physical_tile_loc{37, 18, 0} && loc_b == t_physical_tile_loc{37, 17, 0}) {
        VTR_LOG("%d %d\n", first_id, second_id);
    }
